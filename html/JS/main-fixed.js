// Simplified and robust main.js
console.log('🚀 Loading main-fixed.js...');
...
function setupFilterButtons() {
    ...
    console.log('Filter elements found:', {
        toggleButton: !!toggleFiltros,
        filterModal: !!filtros,
        closeButton: !!filterClose
    });

    // Toggle filters - BASIC TEST WITH ALERT
    console.log('🔍 Looking for filter elements...');
    console.log('toggleFiltros element:', toggleFiltros);
    ...
    if (toggleFiltros && filtros) {
        ...
        toggleFiltros.addEventListener('click', function (e) {
            ...
            // alert('¡Botón de filtros clickeado!'); // Removed for cleaner UX
            console.log('🔄 Filter toggle clicked - BASIC TEST');

            // Debug the filtros element
            console.log('filtros element:', filtros);
            console.log('filtros exists?', !!filtros);
            console.log('filtros tagName:', filtros ? filtros.tagName : 'null');
            console.log('filtros id:', filtros ? filtros.id : 'null');
            console.log('filtros current style.display:', filtros ? filtros.style.display : 'null');

            if (!filtros) {
                alert('ERROR: El elemento filtros no existe!');
                return;
            }

            // Show filters with proper styling
            console.log('📖 Showing filters with proper design');

            // Remove problematic classes
            filtros.classList.remove('filtros-ocultos');
            filtros.classList.add('filtros-mostrados');

            // Set proper inline styles - FIXED POSITIONING
            ...
            console.log('✅ Filters should now be visible with proper styling');
        });
        console.log('✅ Event listener added successfully');
    } else {
        console.error('❌ Filter toggle elements not found!');
        if (!toggleFiltros) console.error('toggleFiltros is null');
        if (!filtros) console.error('filtros is null');
    }

    // Setup close button functionality (using existing filterClose variable)
    ...
    // Close filters when clicking outside
    document.addEventListener('click', function (e) {
        if (filtros && !filtros.contains(e.target) && !toggleFiltros.contains(e.target)) {
            if (filtros.classList.contains('filtros-mostrados')) {
                console.log('🖱️ Clicked outside, closing filters');
                filtros.classList.add('filtros-ocultos');
                filtros.classList.remove('filtros-mostrados');
            }
...
function aplicarFiltros() {
    ...
    const productosFiltrados = productos.filter(producto => {
        if (producto.es_variante) return false; // Skip variants
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    }, 1000); // Wait for other elements to load
});